...
void togglehorizontalmax(const char *arg);
void toggleverticalmax(const char *arg);
...
buttonpress(XEvent *e) {
	...
	if(ev->window == barwin) {
		...
		if(ev->x == 0) {
			switch(ev->button) {
			case Button4: setmwfact("-0.1"); break;
			case Button5: setmwfact("+0.1"); break;
			}
		}
		...
		if(ev->x < x + blw)
			switch(ev->button) {
			case Button1: setlayout(NULL); break;
#ifdef NMASTER
			case Button3: clientspertag("^1"); break;
			case Button4: setnmaster("+1"); break;
			case Button5: setnmaster("-1"); break;
#else
			case Button3: togglemax(NULL); break;
#endif
		} else
		if(ev->x > x + blw)
			switch(ev->button) {
